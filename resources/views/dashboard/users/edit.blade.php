...
@section('container')
    ...
    <div class="col-lg-8">
        ...
        <form action="/users/{{ $user['uid'] }}" method="post" class="mb-5">
            @method('put')
            @csrf
            <div class="mb-3">
                ...
                <input type="name" name="name" class="form-control @error('name')
                    is-invalid
                @enderror" id="name" value="{{ $user['name'], old('name') }}">
                @error('name')
            ...
            <label for="email" class="form-label">Email</label>
                <input type="email" name="email" class="form-control @error('email')
                    is-invalid
                @enderror" id="email" value="{{ $user['email'], old('email') }}">
                @error('email')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3">  
                ...
                <input type="phone" name="phone" class="form-control @error('phone')
                    is-invalid
                @enderror" id="phone" value="@if ($user['mobileNumber'] != 'null')
                    {{ $user['mobileNumber'], old('phone') }}
                @else
                    {{ old('phone') }}
                @endif">
                @error('phone')
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control" id="address" name="address" rows="3">@if ($user['address'] != 'null')
            ...
            <div class="mb-3">
                ...
                <select class="form-select" name="department_id">
                    ...
                    @foreach ($departments as $department)
                        @if (old('department_id') == $department['id'] || $user['departmentId'] == $department['id'])
            ...
            <div class="mb-3">
                ...
                <select class="form-select" name="userProfile_id">
                    ...
                    @foreach ($userProfiles as $userProfile)
                        @if (old('userProfile_id') == $userProfile['id'] || $user['profile_id'] == $userProfile['id'])
